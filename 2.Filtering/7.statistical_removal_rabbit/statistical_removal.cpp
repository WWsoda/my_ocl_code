...
main(int argc, char** argv)
{
	...
	reader.read<pcl::PointXYZI>("rabbit.pcd", *cloud);
	...
	writer.write<pcl::PointXYZI>("rabbit_inliers.pcd", *cloud_filtered, false);
 
	sor.setNegative(true);
	sor.filter(*cloud_filtered);
	writer.write<pcl::PointXYZI>("rabbit_outliers.pcd", *cloud_filtered, false);
	
	std::cout << "运行时间:" << time.getTime() << "ms（毫秒）" << std::endl;
	std::cout << "运行时间:" << time.getTimeSeconds() << "s（秒）" << std::endl;
 
	// 可视化
	reader.read<pcl::PointXYZI>("./rabbit_inliers.pcd", *cloud_inliers);
	reader.read<pcl::PointXYZI>("./rabbit_outliers.pcd", *cloud_outliers);
