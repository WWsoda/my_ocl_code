...
int main (int argc, char** argv)
{
  ...
  range_image.integrateFarRanges (far_ranges); //把超出传感器范围的点并入到深度图像中
  if (setUnseenToMaxRange)
    range_image.setUnseenToMaxRange (); //所有不能观察到的点都为远距离的点
  // --------------------------------------------
  // -----打开三维浏览器并添加点云-----
  // --------------------------------------------
  ...
  border_extractor.compute (border_descriptions); //  提取边界并将结果保留在border_descriptions中
  // ----------------------------------
  // -----在三维浏览器中显示点集-----
  // ----------------------------------
